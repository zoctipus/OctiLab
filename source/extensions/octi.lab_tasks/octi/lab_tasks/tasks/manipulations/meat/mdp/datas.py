...
def update_data(
    ...
    # chop_mid_position_w = _from_eeframes_get_tip_mid_pos_w(fixed_chop_frame, free_chop_frame)
    ...
    dict = {
        "canberry_position": canberry_position,
...
def update_history(
    ...
    robot: Articulation = env.scene[robot_name]
    canberry: RigidObject = env.scene[canberry_name]
    canberry_scale: torch.Tensor = torch.tensor(canberry.cfg.spawn.scale, device=env.device)
    cake: RigidObject = env.scene[cake_name]
    cake_scale: torch.Tensor = torch.tensor(cake.cfg.spawn.scale, device=env.device)
    ee_frame: FrameTransformer = env.scene["ee_frame"]
    ...
    dict = {"end_effector_speed": end_effector_speed, "ee_position_b": ee_position_b}
    return dict

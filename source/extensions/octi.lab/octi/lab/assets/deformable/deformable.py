...
from .deformable_data import DeformableData

if TYPE_CHECKING:
    from .deformable_cfg import DeformableCfg
...
class Deformable(AssetBase):
    ...
    def update(self, dt: float):
        ...
        self._data.root_nodal_positions = self.root_physx_view.get_sim_nodal_positions()
        self._data.root_nodal_velocities = self.root_physx_view.get_sim_nodal_velocities()
        self._data.root_state_w[:, :3] = torch.mean(self._data.root_nodal_positions, dim=1)
        self._data.root_state_w[:, 7:10] = torch.mean(self._data.root_nodal_velocities, dim=1)
    ...
    def write_root_pose_to_sim(self, root_pose: torch.Tensor, env_ids: Sequence[int] | None = None):
        ...
        init_state_pos_tensor = torch.tensor([self.cfg.init_state.pos], device=self.device)
        new_nodal_positions = self.data.default_nodal_positions + \
            (root_poses_xyzw[:, None, :3] - init_state_pos_tensor[:, None, :3])
        # set into simulation
        self.root_physx_view.set_sim_nodal_positions(new_nodal_positions, indices=physx_env_ids)
    ...
    def write_root_velocity_to_sim(self, root_velocity: torch.Tensor, env_ids: Sequence[int] | None = None):
        ...
        # set into simulation
        new_nodal_velocities = self.data.default_nodal_velocities + root_velocity[:, None, :3]
        # set into simulation
        self.root_physx_view.set_sim_nodal_velocities(new_nodal_velocities, indices=physx_env_ids)
    ...
    def _initialize_impl(self):
        ...
        root_prims = sim_utils.get_all_matching_child_prims(
            template_prim_path, predicate=lambda prim: 'PhysxDeformableAPI' in prim.GetAppliedSchemas()
        )
        if len(root_prims) != 1:
            raise RuntimeError(
                f"Failed to find a single deformable when resolving '{self.cfg.prim_path}'."
                f" Found multiple '{root_prims}' under '{template_prim_path}'."
            )
        # resolve root prim back into regex expression
        root_prim_path = root_prims[0].GetPath().pathString
        self.prim_path = root_prim_path
        root_prim_path_expr = self.cfg.prim_path + root_prim_path[len(template_prim_path):]
        # -- object view
        self._root_physx_view = self._physics_sim_view.create_soft_body_view(root_prim_path_expr.replace(".*", "*"))
        # record the root positions
        self._data.default_nodal_positions = self._root_physx_view.get_sim_nodal_positions().clone()
        # it is very interesting the the default_nodal_position is random for
        self._data.default_nodal_velocities = self._root_physx_view.get_sim_nodal_velocities().clone()
        # log information about the articulation
        carb.log_info(f"Deformable initialized at: {self.cfg.prim_path} with root '{root_prim_path_expr}'.")

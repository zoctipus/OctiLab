...
def reset_robot_to_default(env: ManagerBasedEnv, env_ids: torch.Tensor, robot_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
    ...
    robot.write_root_state_to_sim(default_root_state, env_ids=env_ids)  # type: ignore
    ...
    # set into the physics simulation
    robot.write_joint_state_to_sim(default_joint_pos, default_joint_vel, env_ids=env_ids)  # type: ignore
...
def reset_joints_by_offset(
    ...
    joint_pos[:, joint_ids] += math_utils.sample_uniform(*position_range, joint_pos.shape, joint_pos.device)[:, joint_ids]  # type: ignore
    joint_vel += math_utils.sample_uniform(*velocity_range, joint_vel.shape, joint_vel.device)  # type: ignore

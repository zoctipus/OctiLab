...
def eval_env(simulation_app, args_cli, env_cfg):
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            joint_pos_target.append(env.unwrapped.scene["robot"].data.joint_pos_target.cpu().numpy())  # type: ignore
            current_pos_target.append(env.unwrapped.scene["robot"].data.joint_pos.cpu().numpy())  # type: ignore
            applied_torque.append(env.unwrapped.scene["robot"].data.applied_torque.clone())  # type: ignore
            positions.append(env.unwrapped.scene["robot"].data.body_pos_w[0, -1, :3].cpu().numpy())  # type: ignore
            count += 1
...
def main():
    ...
    agent_update_dt = env_cfg.sim.dt * env_cfg.decimation  # type: ignore
    env_cfg.decimation = args_cli.decimation  # type: ignore
    env_cfg.sim.dt = agent_update_dt / env_cfg.decimation  # type: ignore

...
class CranberryDecoratorSm:
    ...
    def initialize(self, dt: float, num_envs: int, device: torch.device | str = "cpu"):
        ...
        self.default_hand_quat[:] = torch.tensor([-0.4000, -0.3300,  0.0700, -0.0126809, 0.7234, -0.6786616, 0.1262941], device = device)
        self.tilted_hand_quat[:] = torch.tensor([-0.4000, -0.3300,  0.0700, -0.0126809, 0.7234, -0.6786616, 0.1262941], device = device)
    ...
    def compute(self, env):
        ...
        canberry_above_cake_distance = general_mdp.get_body1_body2_distance(cranberry, cake, canberry_offset_tensor, above_cake_offset_tensor).view(env.unwrapped.num_envs, -1)
        canberry_height = env.unwrapped.data_manager.get_active_term("data", "canberry_position")[:, 2].view(env.unwrapped.num_envs, -1)
        ...
        ee_berry_x_alignment = (torch.abs(tcp_rest_position[:,0] - cranberry_position[:, 0]) < 0.0065).view(env.unwrapped.num_envs, -1)
        ...
        grasp_mask = (ee_berry_x_alignment & (cranberry_eeframe_distance < 0.008) & (torch.abs(canberry_height - mid_chop_pos_height) < 0.007)).view(-1)
        approach_above_cake_mask = ((chop_pose < -0.45) & (canberry_above_cake_distance > 0.03) & (cranberry_eeframe_distance < 0.02)).view(-1)
